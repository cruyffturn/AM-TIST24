...
def get_E(A):
    ...
    '''
    Returns the type of the each possible edge implied by the graph G
    In:
        A:  p,p
...
def get_adv_err(A_p, A_a, A_est_all, 
    ...
    Compares the types of all edges in G_hat and G_alpha
        ...
        if n_jobs == 1:
            ...
            for i in range(n_rep):
                ...
                if type_noise == 'normal' and type_scm == 'linear':
                    ...
                    load = _missdag(X, cat[:,i],  bool_full, 
                                    idx_mask = idx_mask,
                                    seed = seed_l[i],
                                    type_scm = type_scm
                                    )
...
def _missdag(X, cat,  
    ...
    W, cov_est = helper_missdag_wrap.estimate_missdag_wrapped(X_miss, 
                                                              seed,
                                                              type_scm=='linear')
...
def legacy_get_shd_sub_graph(A_p, A_a, 
                      A_ref,
                      A_est_all, 
                      allow_pdag = False,
                      return_err = False):
    ...
    Out:
        ...
        err:        M,K
    '''
    idx_r, idx_c = np.where(A_p==A_a)   #sub graph matches in two graphs
    
    
#    temp_0 = A_est_all[:,idx_r,idx_c]    
#    temp_1 = A_est_all[:,idx_c,idx_r]
    
    temp = np.stack([A_ref[idx_r,idx_c],
                     A_ref[idx_c,idx_r]],1)
    ...
    temp = []
    for E_i in E_est_all:
        temp.append(_get_pdag_dist(E_ref, E_i, allow_pdag = allow_pdag,
                                   return_err = return_err))
        
    
    n_err = np.stack(temp,0)
#    import ipdb;ipdb.set_trace()
    err_rate = np.mean(n_err)/2
    
    import ipdb;ipdb.set_trace()
    return err_rate

def get_shd_sub_graph(A_p, A_a, 
                      A_ref,
                      A_est_all, 
                      allow_pdag = False,
                      return_err = False):
    ...
    '''        
    Calculates the number of errors in the subgraph where A_a and A_p match
    
    Inter:
        bool_not_sub_graph:     K,      #edges differ A_a vs A_p
        bool_error_i:           K,      #edges differ A_est_i vs A_p
    
    Out:        
        n_err_l:        M,
    '''
    print('using updated sub graph distance')
    bool_not_sub_graph = get_pdag_dist_single(A_p, A_a, 
                                          allow_pdag = False, 
                                          return_err = True)[-1].astype(bool)
...
#    bool_check    
    n_err_l = []
    for A_est_i in A_est_all:
        bool_error_i = get_pdag_dist_single(A_ref, A_est_i, 
                                     allow_pdag = allow_pdag, 
                                     return_err = True)[-1].astype(bool)
        
        n_err_l.append(np.sum(bool_error_i & (~bool_not_sub_graph)))
            
#    import ipdb;ipdb.set_trace()
    err_rate = np.mean(n_err_l)
