...
#from External.MissDAG.dag_methods import Notears_ICA
#from External.MissDAG.utils.utils import postprocess
import helper_missdag_wrap
...
label8 = r'HD($\hat{\mathcal{G}}_{MCAR},\hat{\mathcal{G}}_{MNAR}$)'
label9 = r'subgraph HD($\hat{\mathcal{G}},\mathcal{G}_{ref}$)'
label10 = r'subgraph HD($\hat{\mathcal{G}}_{MCAR},\hat{\mathcal{G}}_{MNAR}$)'
hue = r'Init./$\epsilon$'
...
def get_baseline(X,
    ...
    if type_noise == 'normal' and type_scm == 'linear':
        loss_type = 'l2'        #aka likelihood type
        ...
        print('lambda1 not used')
        
        W_nt, A_nt = helper_missdag_wrap.estimate_baseline_wrapped(X, seed_baseline, 
                                                         w_threshold,
                                                         bool_linear = type_scm == 'linear')
...
#    avg_deg = np.mean(np.sum(A_p,0))    #?
    avg_deg = np.mean((A_p.sum(0)+A_p.sum(1)))
...
#                     edges=edges, 
                     ...
                     width=weight_l,
                     ax=ax)
#    plt.figtext(0.1, .8, '(s,t)=(%i,%i)'%(idx_s,idx_t), wrap=True, 
...
#                                 connectionstyle="arc3,rad=0.1"
    ...
    ax.set_title(title)
    plt.figtext(0.5, 1, '(s,t)=(%i,%i)'%(idx_s,idx_t), wrap=True, 
                horizontalalignment='center', 
                fontsize=12)
...
#                     edges=edges, 
                     ...
                     ax=ax)
#    import ipdb;ipdb.set_trace()
    edge_labels = dict([((n1, n2), '%.2f'%W[n1,n2])
                    for n1, n2 in G.edges if n2 == idx_t])
...
#import matplotlib.pyplot as plt
    ...
    for (A_in, W_in, ax,str_) in zip([A,A_a],[W,W_a],axs,['True Graph',
                                                    'Adversarial Graph']):
        pos = draw_g3(A_in, W_in, 
                      diff_A,                                       
                            str_,
                            idx_t,
                            ax_in = ax,
...
def save_rel_mcar(A_p,
    ...
    savePath2_0 = get_savePath(savePath,
                             0, 
                             exp_type,
                             bool_short = bool_short)
    
    with open(os.path.join(savePath2_0,'A_est.p'), "rb") as f:
        A_est_all_l_0, hue_l_0 = pickle.load(f)
        
    savePath2_1 = get_savePath(savePath,
                             1, 
                             exp_type,
                             bool_short = bool_short)
    
    with open(os.path.join(savePath2_1,'A_est.p'), "rb") as f:
        A_est_all_l_1, hue_l_1 = pickle.load(f)
    ...
    for A_est_all_0, A_est_all_1, hue_i in zip(A_est_all_l_0,
                                               A_est_all_l_1, 
                                               hue_l_0):
        
        if exp_type == 0:
            init_mode, eps = hue_i            
            A_est_all_0 = (np.abs(A_est_all_0)>0).astype(int)
            A_est_all_1 = (np.abs(A_est_all_1)>0).astype(int)
...
#        import ipdb;ipdb.set_trace()
        ...
        if len(A_est_all_0) != len(A_est_all_1):
            raise ValueError
        else:
            n_rep = len(A_est_all_0)
            
        for A_est_all_0_i, A_est_all_1_i in zip(A_est_all_0, A_est_all_1):
            
            n_total_rel_i = helper.get_pdag_dist(A_est_all_1_i, 
                                               A_est_all_0_i[np.newaxis], 
                                               allow_pdag=allow_pdag)

                    
            temp = helper.get_pdag_dist(A_est_all_0_i, 
                                        A_est_all_1_i[np.newaxis], 
                                        allow_pdag=allow_pdag)
            
            if np.any(temp != n_total_rel_i): #Sanity check
                raise ValueError
            
            n_total_sub_rel_i = helper.get_shd_sub_graph(A_p, A_a, 
                                                   A_est_all_1_i, 
                                                   A_est_all_0_i[np.newaxis], 
                                                   allow_pdag = allow_pdag)            
            
            temp = helper.get_shd_sub_graph(A_p, A_a, 
                                           A_est_all_0_i, 
                                           A_est_all_1_i[np.newaxis], 
                                           allow_pdag = allow_pdag)
            
                    
            if np.any(temp != n_total_sub_rel_i): #Sanity check
                raise ValueError
                
            n_total_rel.append(n_total_rel_i)
            n_total_sub_rel.append(n_total_sub_rel_i)
        
#        import ipdb;ipdb.set_trace()
        print('n_rep',n_rep)
        n_total_rel = np.hstack(n_total_rel)
        n_total_sub_rel = np.hstack(n_total_sub_rel)
        
        n_total_p_0 = helper.get_pdag_dist(A_p, A_est_all_0,
                                           allow_pdag=allow_pdag) #for reference.        
        
        n_total_p_1 = helper.get_pdag_dist(A_p, A_est_all_1,
                                           allow_pdag=allow_pdag) #for reference.        
                        
        df = pd.DataFrame({label8:n_total_rel,
                           label10:n_total_sub_rel,
                           'reference_mnar':n_total_p_0,
                           'reference_mcar':n_total_p_1,
                           'n_rep':n_rep
                           })
    ...
    summary = pd.concat([df_mean,df_std], axis = 1)
    summary.to_csv(os.path.join(savePath2_0, 'mcar_rel_summary.csv'))
    
def save_rel_sub(A_pc, A_nt, 
                 A_p, A_a,
                 savePath,
                 exp_type, bool_mcar,
                 bool_short = False
...
#        import ipdb;ipdb.set_trace()
        n_total_new = helper.get_shd_sub_graph(A_p, A_a, 
                                               A_ref, A_est_all, 
                                               allow_pdag = allow_pdag)
...
#            import ipdb;ipdb.set_trace()
    ...
    summary = pd.concat([df_mean,df_std], axis = 1)
    summary.to_csv(os.path.join(savePath2,'subgraph_rel_summary.csv'))