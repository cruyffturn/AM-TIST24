            ...
            for seed_i in range(n_init):
                with tf.GradientTape() as tape:
                    ...
                    if not bool_scale:
                        ...
                        scale_mu = np.mean(x,0)
                        scale_std = np.std(x,0)
                        scale_std[self.idx_adv[1]] = 1
                        x_in = (x - scale_mu)/scale_std
                        print('new scaling')
#                        z_scaled = (z - self.scale_mu)/self.scale_std
                        
                    if not self.bool_sub:
                        p_r_x = self(x_in, training=True)  # Forward pass
                    else:
                        x_sub = tf.constant(x_in.numpy()[:,self.idx_adv])                        
...
#                        S_new_ = np.linalg.inv(K_new)
                        ...
                        P_a = helper_dag.get_pear(self.S_a)
                        
                        if len(self.idx_adv) <3:
                            print(seed_i, 
                                  '[0,1]: %.2f (%.2f)'%(
                                  P_new[self.idx_adv[0],self.idx_adv[1]],
                                  P_a[self.idx_adv[0],self.idx_adv[1]]))
#                            print(seed_i, 
...
#                                  )
                            ...
                            print(seed_i, 
                                  ...
                                  P_new[self.idx_adv[0],self.idx_adv[1]],
                                  P_a[self.idx_adv[0],self.idx_adv[1]],
                                  P_new[self.idx_adv[0],self.idx_adv[2]],
                                  P_a[self.idx_adv[0],self.idx_adv[2]])
                                  )
